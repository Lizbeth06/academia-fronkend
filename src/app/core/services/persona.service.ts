...
export class PersonaService extends GenericService<Persona> {
  ...
  constructor(protected override http: HttpClient) {
    super(http, `${environment.HOST}/api/persona`);
  }

  findTipodocByNumdoc(tipodocumento: number, numdocumento: string) {
    const params = new HttpParams().set("tipodocumento", tipodocumento).set("numdocumento", numdocumento);
  ...
  updateByCorreoTelefonoByIdpersona(idPersona: number, correo: string, telefono: string) {
    const params = new HttpParams().set("idPersona", idPersona.toString()).set("correo", correo).set("telefono", telefono);
